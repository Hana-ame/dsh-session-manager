...
| ② 管理工具 | `package/lib/tools.mjs` | 10 个会话管理工具：读写 ①、指挥本进程内的其他会话 |
...
│   ├── tools.mjs                    # ② 10 个管理工具（preset 挂载这一层）
...
| `session_queue` | **只读**列出某个会话当前排队中的消息：顺序即投递顺序，每条带 `queued` / `steering` 标注。队列属于活着的 agent，冷会话没有队列；不投递、不取消、不放行 |
...
- **关系与备注同库**：`session_list` 观测到的血统（②）与 `session_describe` 写的备注（②）都进这一份文件，画布（③）读的也是它——所以「工具」和「画布」是同一份记录的两个视图。
- **读**：每次先 `stat` 文件（mtime+size），变了才重读，因此就算有第二个模块实例/进程在写也不会读到陈旧缓存。
- **写**：串行化 + 临时文件 `rename`，崩溃不会留下半截文件；**没有变化就不写**（重复 `session_list` 不会反复落盘）。
...
```sh
node package/test/tools.test.mjs     # 工具层 61 项
node package/test/scope.test.mjs     # 画布作用域 18 项
```

工具层用假 Cordis ctx + 临时 `DSH_HOME` 跑真实插件文件，覆盖：① 的迁移 / 落盘 / 不再重复落盘 / 清备注保留血统，`session_list` 的血统标注、备注、模型路由与观测落库，`session_read` 三档 detail，模型目录的渲染与失败项，模型**读取**（优先 projection、回退 fold、不 resume）与**切换**（字段完整、半对参数被拒、副作用披露），发送/自投递/空文本拦截、取消、分叉与自动命名，排队消息读取（顺序、`placement`、截断、空队列、冷会话、读完即释放控制流、只读性），以及压缩（替目标跑 `/compact`、传出真实信号与空附件、拒绝结果、无压缩 preset、冷会话、自压缩拦截）。
